...
module.exports = {
  ...
  quoteProps: 'consistent', // 引用对象中的属性时更改    //这里没有测试出来
  ...
  rangeStart: 0, //回退至包含选中语句的第一行的开始 //不起作用

  rangeEnd: Infinity, //向前直到选中语句的末尾 //不起作用

  requirePragma: false, //是否严格按照文件顶部的一些特殊的注释格式化代码

  insertPragma: true, //是否插入文件顶部的一些特殊的注释格式化代码

  proseWrap: 'always', //是否是当屏幕放不下时发生的软折行 always（当超出print width宽度时折行）|never|perserve(按照文件原样) //不起作用

  htmlWhitespaceSensitivity: 'strict', //设置回车符是否换行 //不起作用

  endOfLine: 'cr' //包含Linux风格的行结尾 enter 和 ctrl+enter //不起作用
};
